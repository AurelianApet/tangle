...
function animateFlow(canvasName, speed, text = "Mingle and Wiggle") {
    ...
    var xoffset = [0, 0];
    var yoffset = [window.innerWidth * 0.139, window.innerWidth * 0.22];


    context.font = "900 " + window.innerWidth * 0.064 + "px Pretendard";
    ...
    setInterval(function() {
        ...
        for (id = 0; id < 2; id++) {
            ...
            for (i = 0; i < allText.length; i++) {
                ...
                if (!(x < -window.innerWidth * 0.278 || x > window.innerWidth + window.innerWidth * 0.278)) {
                    y = Math.cos(wSpeed * x) * - 10 + yoffset[id];
                ...
                if (x > window.innerWidth + window.innerWidth * 0.278) break;
            }
        }
    }, 40);
}
...
$(window).scroll(function() {
    if ($(this).scrollTop() > window.innerWidth * 1.11) {
    ...
    if ($(this).scrollTop() > window.innerWidth * 2.22) {
        if (!mask_state) {
            mask_state = true;
            setInterval(function () {
                if (mask_left < window.innerWidth * 0.278) {
    ...
    if ($(this).scrollTop() > window.innerWidth * 4.167) {
        if (!mingleMaskLeft1_state) {
            mingleMaskLeft1_state = true;
            setInterval(function () {
                if (mingleMaskLeft1 < window.innerWidth * 0.28) {
    ...
    if ($(this).scrollTop() > window.innerWidth * 4.72) {
        $('.mingle').addClass('mingleWiggleAnimation');
        $('.tangle').addClass('mingleWiggleAnimation');
    }

    if ($(this).scrollTop() > window.innerWidth * 5.56) {
        if (!productMask1_state) {
            productMask1_state = true;
            setInterval(function () {
                if (productMaskLeft1 < window.innerWidth * 0.28) {
    ...
    if ($(this).scrollTop() > window.innerWidth * 6.94) {
        if (!productMask2_state) {
            productMask2_state = true;
            setInterval(function () {
                if (productMaskLeft2 < window.innerWidth * 0.28 && $('.productMaskLeft2') && $('.productMaskLeft2')[0]) {
...
$('.body-container-first-pane-bottom').click(function () {
    ...
    if (img_state == 1) {
        ...
        setInterval(function () {
            if (img_mask_left < window.innerWidth * 0.278) {
